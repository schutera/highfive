...
- **Never ship the dev API key (`hf_dev_key_2026`)** as a production fallback. Override `HIGHFIVE_API_KEY`. (Self-enforced at backend startup by `backend/src/auth.ts`'s `DEV_FALLBACK_KEY` guard for any `NODE_ENV` outside the small safelist `['', 'development', 'test']` in `backend/src/env.ts`'s `DEV_ENV_TOKENS` — `'staging'`, `'qa'`, `'production'`, and unrecognised values all trip the guard. Symmetric load-time guard for the frontend in `homepage/src/services/api.ts`'s `validateBuildTimeApiKey`. The prose entry remains because `NODE_ENV=development` is an intentional dev-mode off-ramp — a container deployed with that value to a production host is on the operator. See [ADR-003](docs/09-architecture-decisions/adr-003-shared-api-key-for-admin.md) and [auth.md → "The secret"](docs/08-crosscutting-concepts/auth.md#the-secret).)
