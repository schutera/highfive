...
Conventions and orientation for the **HiveHive** (a.k.a. `highfive`) bee-monitoring monorepo. For deeper context see [`ARCHITECTURE.md`](ARCHITECTURE.md) and [`CONTRIBUTING.md`](CONTRIBUTING.md) — this file does not duplicate them.

## Project at a glance

HiveHive monitors wild-bee nesting activity: ESP32-CAM modules upload images to a Python image service, a Python DuckDB service owns persistence, a Node/Express backend aggregates for the UI, and a React + Vite homepage renders dashboard, map, and setup wizard. Everything dev-side runs under `docker compose` on a shared bridge network `net`.
...
Backend Swagger UI: <http://localhost:3002/api-docs>. The `duckdb-service` auto-seeds five sample modules when `SEED_DATA=true` (compose default) and the DB is empty.
...
```bash
cd backend        && npm ci && npm test                       # vitest + supertest, 21 tests
cd homepage       && npm ci && npm test                       # vitest + jsdom, 8 smoke tests
cd duckdb-service && pip install -r requirements-dev.txt && pytest tests/ -q   # 15 tests
cd image-service  && pip install -r requirements-dev.txt && pytest tests/ -q   # 24 tests
...
- **Field-name drift**: there has been historical typo drift around `progess`/`progress`. When touching DB/API field names, grep for both spellings before changing things.
- **Conventional Commits**: first line `<type>: <imperative summary>` ≤ ~72 chars. Allowed types: `feat`, `fix`, `refactor`, `chore`, `test`, `ci`, `docs`. Body explains *why*, not *what*.
...
Concurrency is set to cancel superseded runs on the same ref. Workflow also runs on pushes to `chore/test-harness`.
...
- **Never `--amend`** a commit when a pre-commit hook fails — the commit didn't happen, so amend would clobber the *previous* commit. Re-stage and create a new commit.
