...
- **Never ship the dev API key (`hf_dev_key_2026`)** as a production fallback. Override `HIGHFIVE_API_KEY`. (Self-enforced at backend startup under `NODE_ENV=production` by `backend/src/auth.ts`'s `DEV_FALLBACK_KEY` guard and at frontend build time by `homepage/src/services/api.ts`'s `validateBuildTimeApiKey`. The prose entry remains because partial dev-mode coverage still relies on operator discipline — see [ADR-003](docs/09-architecture-decisions/adr-003-shared-api-key-for-admin.md) and [auth.md → "The secret"](docs/08-crosscutting-concepts/auth.md#the-secret).)
