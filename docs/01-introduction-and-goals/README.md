...
- Not a generic image-classification platform. A learned detector
  already localizes holes for the per-nest snips (ADR-027), but the
  empty/sealed classifier is still a stub; a learned classifier is
  planned and the data shape (cell counts per nest per day) is fixed.
