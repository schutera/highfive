...
flowchart TD
    ...
    OpenMeteo["Open-Meteo<br/>(activity chart, browser-direct)"]
...
| **Open-Meteo**                    | outbound (browser-side)      | `GET https://api.open-meteo.com/v1/forecast?...` from `homepage/src/services/weather.ts`'s `fetchHourlyWeather`, one call per `ActivityWeatherChart` mount | Module lat/lng (already a public, fuzzed value) → hourly temperature + precipitation. No key required; CORS open. Call is browser-direct, **not** proxied through the HiveHive backend. Degrades to "weather unavailable" hint when the API is unreachable. See [ADR-015](../09-architecture-decisions/adr-015-weather-correlation.md). |
