...
| `GET`  | `/images`               | newest-first upload metadata pages: the admin gallery in `AdminPage.tsx` (paginated, always on) and — only when `VITE_ENABLE_DASHBOARD_IMAGES=true` (ADR-022, default off) — the public "Latest captures" carousel in `LatestCaptures.tsx` (paged, two 4:3 cards visible + arrows) |
