...
| `hf::tls::kGoogleApisCaBundlePem` (R1 **and** R4) in the same header                                            | Google issuance for `www.googleapis.com`           |

> **2026-06-05 update (CA rotation).** `www.googleapis.com` rotated its served chain from `…->WR2->GTS Root R1` (RSA) to `…->WE2->GTS Root R4` (ECC). The geolocation call originally pinned `kGtsRootR1Pem` alone, so after the rotation **every** `getGeolocation` TLS handshake failed peer verification → the firmware fell through to the `(0,0)` sentinel and every module showed "Standort ausstehend" on the map, indefinitely. Fix: pin `kGoogleApisCaBundlePem`, a concatenation of GTS Root R1 + R4 (`setCACert` accepts a multi-PEM bundle; mbedTLS treats each block as an independent trust anchor), so the handshake verifies against whichever chain Google serves and survives a rotation in either direction. Shipped in firmware `longhorn` / OTA sequence 3. Lesson recorded in [chapter 11](../11-risks-and-technical-debt/README.md).
...
- [`ESP32-CAM/esp_init.cpp`'s `attemptGeolocation`](../../ESP32-CAM/esp_init.cpp) — geolocation. URL is hardcoded `https://www.googleapis.com/...` so the dispatch is unconditional TLS, pinned to the `kGoogleApisCaBundlePem` (GTS Root R1 + R4) bundle — see the 2026-06-05 CA-rotation note above.
...
**Enables.** The `HIGHFIVE_API_KEY`, image bodies, telemetry, and the WiFi-fingerprint geolocation request all become opaque to any passive listener on the WiFi path. (An earlier draft of this ADR additionally claimed the `/new_module` payload carries an operator email — the `esp_config->email` struct field exists in [`ESP32-CAM/esp_init.cpp`'s `initNewModuleOnServer`](../../ESP32-CAM/esp_init.cpp) and is JSON-encoded conditionally behind a `strlen > 0` check, but [`ESP32-CAM/host.cpp`'s `sendConfigForm`](../../ESP32-CAM/host.cpp) does not surface an input for it, so in practice the field is always empty and ArduinoJson omits the key. Verified against the 2026-05-15 smoke test, where module `fierce-apricot-specht` registered successfully without an operator email being present in the request body.) A MITM attempting to substitute a server presents a cert the firmware refuses to verify against ISRG Root X1, breaking the handshake before any byte of the request leaves.

**Costs.** Each TLS handshake costs ~30–50 KB heap transiently and ~1–2 s wall-clock on the wire. The 60 s `TASK_WDT_TIMEOUT_S` budget in [`ESP32-CAM/ESP32-CAM.ino`'s `setup`](../../ESP32-CAM/ESP32-CAM.ino) covers it comfortably. The embedded PEMs (ISRG Root X1, plus the GTS Root R1 + R4 googleapis bundle) add ~4 KB to the flash image (RAM is `.rodata`, no heap cost outside the handshake). The `static WiFiClientSecure` in `postImage` is slightly larger than the prior `WiFiClient` — observable as ~50 bytes additional RAM usage in the static segment. Boot-log heap measurement at first flash should confirm the runtime free-heap stays above the ~30 KB floor needed for a clean TLS handshake.
