...
`.github/workflows/tests.yml` runs **nine parallel jobs** on PRs to
...
| `duckdb-unit`   | `pytest tests/ -q` in `duckdb-service/`, across a Python **3.10–3.14** matrix (`fail-fast: false`); 3.10 is prod's floor, 3.14 the forward ceiling — closes the 3.11-only gap that crashed prod (#180). See [ADR-028](../09-architecture-decisions/adr-028-python-version-matrix-floated-pins.md).                                                                                                                                                                                                                                                 |
| `image-unit`    | `pytest tests/ -q` in `image-service/`, across the same Python **3.10–3.14** matrix; native deps (`numpy`, `onnxruntime`) are floated to `>=` so each cell resolves a per-interpreter wheel (no single pin spans cp310…cp314). See [ADR-028](../09-architecture-decisions/adr-028-python-version-matrix-floated-pins.md).                                                                                                                                                                                                                                                  |
