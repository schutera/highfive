...
| `duckdb-unit`   | `pytest tests/ -q` in `duckdb-service/`, across a Python **3.10–3.14** matrix (`fail-fast: false`). The repo specifies its Python runtime inconsistently (Dockerfiles build `3.12-slim`, deploy docs say 3.11, the #180 incident crashed on a 3.10 host), so the matrix spans a conservative 3.10 floor to a 3.14 ceiling — closing the single-version gap that crashed prod (#180). See [ADR-028](../09-architecture-decisions/adr-028-python-version-matrix-floated-pins.md).                                                                                                                                                                                                                                                 |
