...
> | T5 recovery half            | DEFERRED to server-side validation | Server log: `[heartbeat] patched module_configs lat/lng for aabbccddeeff from (0,0) -> (47.79,9.62) acc=50.0`. The firmware-side `[getGeolocation] deferred retry SUCCESS` line was not exercised on hardware (30-min wait + needs WAN-egress break). The server-side "only patch from (0,0)" path is pinned by [`duckdb-service/tests/test_heartbeats_endpoint.py`'s `test_heartbeat_with_lat_lng_patches_null_island_module`](../../duckdb-service/tests/test_heartbeats_endpoint.py) (positive case) + `test_heartbeat_with_lat_lng_does_not_overwrite_placed_module` (negative case — invariant guard), and the wire shape was end-to-end-validated by manual `curl` against the live dev stack on the same 2026-05-17 iteration. |
