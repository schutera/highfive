...
- **Wi-Fi credential** — a commented-out `WiFi.begin("<SSID>",
  ...
  shape is caught **at pre-push time**; as of #210 it is also a CI gate
  (the `repo-guards` job in `.github/workflows/tests.yml`), and it does not cover
...
- `module_configs.lat`/`lng` has two writers — `/new_module`'s
  ...
  (renamed and inverted in the 2026-08 audit, for #229 — see "A UPSERT's
  location guard was carried forward under a comment asserting symmetry
  it didn't have" below; it now pins the OPPOSITE of what its original
  name claimed),
  ...
  **The test files pin the (lat, lng) quadrant transitions but
  ...
  changes. **Since the 2026-08 audit (#229), `/new_module`'s CASE
  gates on whether the STORED row sits at `(0,0)`** — not the
  incoming payload, the opposite of this paragraph's original
  (pre-#229) formulation — while `_is_plausible_fix` is a Python
  predicate on the *incoming* heartbeat fields that also rejects NaN,
  out-of-range, and zero accuracy. The two writers' gating conditions
  now differ in kind, not just in language: one reads `module_configs`,
  the other reads the request body. The convergence of input validity
  still comes from `ModuleData.{latitude,longitude}: Field(ge=…, le=…)`
  Pydantic clamps at the entry point, not from either CASE. A future
  repo-method consolidation must account for both gating conditions AND
  the Python predicate together; treating either SQL CASE alone as the
  seam will leave the asymmetric input validation untouched.
...
### The weekly "backup" was not a backup, and images were never backed up at all (#232)

**What happened.** The 2026-08-18 repo audit (finding OPS-1) found that `duckdb-service`'s Sunday-03:00 job (`services/backup.py`'s `run_backup()`) held the single process-wide `db.connection.lock` — the same lock every route and repository helper acquires (ADR-001) — for the entire duration of gzip-streaming the **whole** live `DB_PATH` into a temp directory, then posted the result to a Discord webhook attachment, then deleted the temp directory. Three separate defects hid behind one green-looking scheduled job: (1) every DuckDB request stalled for the full gzip duration once a week — long enough that `image-service`'s 5 s client timeout could trip and leave uploads on disk with no `image_uploads` row; (2) Discord attachments cap at ~10 MB and the issue recorded prod's DB file at ~8 GB — a figure from an earlier PM2-hosted deployment, not necessarily the current `production` deployment's size (see the "Fix" paragraph below) — so at that scale the "backup" almost certainly never even delivered; (3) the temp directory was deleted either way, so **no copy of the database ever survived anywhere**, and `/data/images` (captures, `snips/`, `*.log.json` sidecars) was never touched by any job at all. `docs/05-building-block-view/duckdb-service.md` had claimed the job wrote "a sibling backup file under `/data`" — false; and neither `backup.py` nor `silence_watcher.py` had a single test, so this had been silently broken since it was first written.

**Why it happened.** The job *looked* like a backup — it ran on schedule, it logged "Starting weekly backup," and a passing Discord post (when a webhook happened to be configured, which it wasn't by default post-#201) looked like success. Nothing ever exercised the failure paths: the Discord-attachment size cap, the lock-hold duration under a prod-sized file, or a real restore. A scheduled job with no test and no drilled restore is a belief, not a guarantee — "it runs" was mistaken for "it works," and the one doc that described it made the same mistake load-bearing by writing it down as fact.

**How to avoid it next time.** A backup job is not done when it runs without throwing — it is done when (a) the retained artifact has been verified to actually restore (opened read-only, row counts checked), (b) a size/transport limit that could silently truncate or drop the artifact has been checked against real data scale, not dev-scale, and (c) the lock-hold / stall cost has been measured, not assumed to be "fast enough." All three were unverified here for roughly the job's entire lifetime. Any code path whose only job is disaster recovery needs its own test suite as a matter of course — untested recovery code is discovered broken exactly when it's needed most.

**Fix — staged, not fully closed.** `run_backup()` now holds `lock` only for a `CHECKPOINT` + `shutil.copy2` (measured ~0.07 s on the dev-seeded DB; **this is a dev-scale number, not a scale-independent guarantee** — the hold is O(DB size), so re-measure before trusting it at prod scale), gzips + sha256's + rotates (`BACKUP_KEEP`, floored at 1, default 4, falling back to the default rather than raising on a non-integer value) outside the lock, aborts with a Discord alert if free space is under 1.5× the live DB size before even attempting the copy, and sends Discord a text-only notification — never the file. See [ADR-031](../09-architecture-decisions/adr-031-backup-file-copy-not-export-database.md), `duckdb-service/tests/test_backup.py`, and [production-deployment.md → Backup & Restore](../07-deployment-view/production-deployment.md#backup--restore) for the restore drill actually performed (dev-stack, row counts verified identical pre/post).

As of this PR, current `production` has no data yet (confirmed directly, not assumed) — so the manual pre-migration backup (Step 0) has been documented and drilled on the dev stack only, not executed against live prod. **The off-host-sync half of #232's original "irreversible-data-loss risk" framing is still open**: retained backups so far live on `BACKUP_DIR`, which is a subdirectory of the **same** `duckdb_data` volume as the live DB, so a volume-level failure still takes both out together. Off-host sync is documented as a systemd-timer template (production-deployment.md, same section) that writes a heartbeat file the app checks (`_has_fresh_offhost_sync`, gating the "local-only" warning on recency rather than a one-time acknowledgement flag) — but nothing has actually run it yet. Don't read this entry as "backup is solved"; read it as "the job that ran was fake, the job that runs now is real and tested, and the off-host leg is a documented-but-unexecuted next step."

### "Content is opaque to the service" was the design, and the test fixture encoded the gap instead of catching it (#228)

**What happened.** The 2026-08-18 repo audit (findings SEC-9, SEC-10, SEC-4) found that `image-service`'s `POST /upload` — internet-reachable, credential-free by design — validated `mac`/`battery` and the presence of a file part, but never inspected the uploaded bytes, never coerced the stored extension, and never bounded declared image dimensions before handing the file to `cv2.imread`. Three related defects followed from the one gap: an uploaded `.html`/`.svg` was stored and later served back with a Content-Type *guessed from its extension* (same-origin stored content on the domain that carries the admin session cookie, ADR-019); a crafted JPEG with a huge SOF-declared frame could OOM-kill the single-process service before any dimension check ever ran; and the `.log.json` telemetry sidecar — written into the same folder the image route serves from — was reachable through `GET /images/<name>.log.json`, bypassing the admin gate on the equivalent telemetry-read endpoint. `image-service/tests/test_upload.py`'s own fixture helper was named `_img_bytes()` and its comment said, verbatim, *"Content is opaque to the service — it does not decode the image"* — a 1×1 **PNG** posted with `filename="test.jpg"`. The test suite didn't just fail to catch the gap; its own fixture *demonstrated* the gap and called it correct.

**Why it happened.** "The service doesn't decode the upload, so content doesn't matter" was true as a description of the code and false as a security posture — those are different claims, and the comment only ever asserted the first one. Once written down as a fixture-level design note, it became the thing new tests were written *against*: any test that tried to post a real, well-formed image would have looked like unnecessary rigor compared to the existing opaque-PNG pattern everyone was already copying. A serve route inherits whatever the client's chosen extension implies unless something upstream forces otherwise — and nothing did, because nothing was checked at the point bytes actually entered the system.

**How to avoid it next time.** A trust-boundary comment that describes current behaviour ("we don't decode this") must not be read as an endorsement of that behaviour being *safe* — say so explicitly if it's actually a known gap, or the next person (or agent) treats the description as the design. Validate content at the point untrusted bytes enter the system (the upload handler), not in the consumer that later serves them back — a serve route must never let the client's declared filename or Content-Type decide what gets sent to a browser. And when a test fixture's own comment states a security-relevant simplification ("content is opaque"), that line is exactly the one to interrogate before extending the pattern to new tests, not the one to copy fastest.

**Fix.** `services/image_guard.py::probe_jpeg` (new) reads JPEG magic bytes and walks header segments to the first SOF marker — without decoding pixel data — before `_persist_image` reserves a filename or saves anything; a rejection is a `400` with nothing written. `services/paths.py::sanitize_upload_filename` now forces every stored extension to `.jpg` regardless of the client-supplied name. `app.py`'s `serve_image`/`serve_snip` refuse any non-`.jpg` name (closing the sidecar-reachability gap as a side effect) and pin `Content-Type: image/jpeg` explicitly rather than letting `send_from_directory` guess it. The test fixture now encodes a real, tiny, on-the-fly JPEG (`cv2.imencode`), and `test_upload.py`/`test_image_guard.py` add explicit negative cases — PNG-as-`.jpg`, oversized SOF, non-JPEG-with-any-extension — so the next fixture change has to actively break a passing assertion, not just extend an unchecked pattern. See [auth.md](../08-crosscutting-concepts/auth.md), [api-contracts.md](../08-crosscutting-concepts/api-contracts.md), and [image-upload-flow.md](../06-runtime-view/image-upload-flow.md).

### Two internet-reachable write routes logged their own inputs before validating or generalizing them (#229)

**What happened.** The 2026-08-18 repo audit (findings SEC-1, SEC-12) found that `duckdb-service`'s `POST /new_module` printed `f"[new_module] Received: {json_data}"` — the **raw, pre-validation** request body, including the operator's `email` and any attacker-supplied extra keys — into the same admin-readable, disk-persisted log ring (ADR-023) that `app.py`'s own module docstring says is "path ONLY — never query string, headers, or body." `POST /heartbeat` had the same shape one field over: it printed `lat={lat} lng={lng} acc={acc}` **before** `coarsen_coord` ran, so sub-2-dp-precision coordinates — the exact precision ADR-020 exists to strip — landed in the ring for up to the retention window. Neither route required a credential (both are proxied to the internet by host-nginx), and re-registering an existing module id silently overwrote its `name`/`email`/`lat`/`lng` from whatever the (anonymous) caller sent, which is a public-map-defacement / Discord-webhook-spam vector independent of the logging issue.

**Why it happened.** Both print statements were written as debug aids at a time when the routes were assumed to be reachable only from trusted in-bridge callers ("unauthenticated by design, on the assumption that only in-bridge callers reach them" — `auth.md`, a claim that was already false for these two routes in production). A `print(f"... {json_data}")` one-liner is the fastest way to see what a handler received, and nothing about writing it flagged that the same line would later be piped into a structured, disk-persisted, admin-readable ring (ADR-023 landed after these print statements did). The coordinate case is subtler: the log line and the `coarsen_coord` call were both present in the function, just in the wrong order — a one-line reordering bug, not a missing control, which is exactly the kind of defect a "no secrets in logs" review checklist item doesn't catch (checklists check for absence of a pattern, not for a correct-looking line running before its own precondition).

**How to avoid it next time.** Treat "logs a request" and "logs pre-validation input" as different claims that need separately verifying — a debug `print` written before a validation/generalization step runs is a claim about *timing*, and timing bugs don't show up by reading the log line in isolation. For any handler on a route the doc says is credential-free or internet-reachable, audit every `print`/`log` call in that function specifically for what's in scope *at that point in the function*, not just what the function eventually validates. And a claim like "assumption holds in production" belongs next to the evidence that makes it true (an nginx bind, a firewall rule) — if that evidence is two paragraphs away describing the opposite, the doc is self-contradicting and the contradiction is the tell.

**Fix.** `routes/modules.py::add_module`'s pre-validation `print` is removed; the replacement line runs after the DB write and logs only the canonical id, the actually-stored name, and already-coarsened lat/lng — never `email`, never the raw body. `routes/heartbeats.py::post_heartbeat`'s log line drops `lat`/`lng`/`acc` entirely in favour of a `geo_present` boolean; the separate geo-patch log line (which fires only when a patch actually applies) already logged the post-`coarsen_coord` result and was left as-is. Re-registration semantics changed independently: `name`/`email` are now preserved unless NULL/empty, and `latitude`/`longitude` are preserved unless the **stored** row sits at the `(0,0)` sentinel and the incoming payload carries a real fix (see the next lesson entry for how the first-pass version of this got the direction backwards); the Discord webhook fires only on first insert, and an unregistered mac's heartbeat is dropped (`200`, nothing written) instead of growing tables for a module that doesn't exist. `duckdb-service/tests/test_modules.py` and `test_heartbeats_endpoint.py` assert directly against `services.log_ring.get_recent(...)` output — not just that the routes don't 500 — so a future regression that reintroduces a pre-validation echo or an out-of-order coordinate print fails a test, not just a review. See [auth.md](../08-crosscutting-concepts/auth.md), [api-contracts.md](../08-crosscutting-concepts/api-contracts.md), and [ADR-032](../09-architecture-decisions/adr-032-device-identity-for-ingest.md) for the device-identity follow-up this PR deliberately did not implement.

### A UPSERT's location guard was carried forward under a comment asserting symmetry it didn't have (#229, caught pre-merge by senior-review)

**What happened.** The first-pass fix for #229's `add_module` re-registration hardening wrote fresh preserve-unless-empty CASE logic for `name` and `email`, but for `lat`/`lng` it kept the **existing** PR-II/#89 SQL verbatim: `WHEN EXCLUDED.lat = 0 AND EXCLUDED.lng = 0 AND NOT (module_configs.lat = 0 AND module_configs.lng = 0) THEN module_configs.lat ELSE EXCLUDED.lat`. That CASE gates on the **incoming** payload being `(0,0)` — it was written to answer "did this boot's GPS fix fail?", and under it any non-`(0,0)` incoming value always overwrites the stored one, exactly the pre-#229 behaviour. A code comment directly above it asserted "Symmetric with `routes/heartbeats.py::post_heartbeat`'s 'only patch from (0,0)' rule" — but `post_heartbeat`'s guard gates on the **stored** row being `(0,0)`, the opposite condition. The two were not symmetric; one was inverted relative to the other. Six documents (`api-reference.md`, `api-contracts.md`, `auth.md`, this chapter, `ADR-032`, and the code's own inline comment) all asserted the non-inverted (safe) behaviour, because they were written to describe the *intent* of #229's spec, not read back against the actual CASE. Senior-review caught it by running the endpoint directly: registering a module at `(47.79, 9.62)` and then re-POSTing with `(-33.86, 151.20)` moved it to Sydney. `duckdb-service/tests/test_modules.py::test_new_module_re_registration_with_real_fix_overwrites_existing` — an **existing, passing** test from before #229 — pinned the vulnerable direction as correct, with a docstring justifying it as "the operator physically moved the module" path; nobody revisited whether that justification still held once the route's actual caller became "anyone on the internet."

**Why it happened.** "Preserve unless empty" is an intuitive, symmetric-sounding rule, and reusing an existing CASE block that already handled "the `(0,0)` sentinel" *felt* like reuse of a solved problem rather than a fresh design decision that needed its own derivation. The reused code even came with a docstring-style comment ("PR II / issue #89 follow-up") that made it look load-bearing and intentional, not like a leftover. Nothing forced a re-derivation of "which side of this CASE should gate on which row" from #229's actual requirement ("keep `lat`/`lng` unless NULL/empty" — and `lat`/`lng` are never NULL, so the real question was "what does 'empty' mean for a coordinate," which the reused code never actually answered). The existing test that pinned the old behaviour gave false confidence: green tests read as "this path is covered," not as "this test was written for a different threat model and needs to be re-examined."

**How to avoid it next time.** When a fix's own prose says two code paths are "symmetric" or "the same rule," write the sentence out as a literal condition for each path and diff them — don't infer symmetry from the fact that both paths mention the same sentinel value. A CASE expression inherited from a prior PR for a *different* purpose (a boot-time GPS-fix recovery mechanic) needs to be re-derived, not reused, when it's repurposed to satisfy a *new* threat model (an anonymous-caller overwrite guard) — the two problems can share a sentinel value and still need opposite gating conditions. And when a security fix touches a table with existing tests, grep for every existing test asserting the OLD behaviour on the field being protected, not just the fields with fresh tests — an old green test can be the exact thing pinning the vulnerability the new fix was supposed to close. Most concretely: **run the endpoint, not just the test suite** — the senior-reviewer caught this by making two real HTTP calls and reading back the row, which took under a minute and found what a full battery of unit tests (written by the same session that introduced the bug) did not.

### "Kept in sync" was a docstring, not a mechanism — and one twin had already drifted (#241)

**What happened.** Five small Python modules are hand-duplicated between `image-service` and `duckdb-service` because Python has no npm-style workspace mechanism: `services/log_ring.py`, `services/prod_guard.py`, `module_id.py` (asymmetrically located — `image-service/services/` vs `duckdb-service/models/`), `services/discord.py`, and `tests/test_prod_guard.py`. Three of them carried module docstrings stating the copies were "kept in sync", and this chapter said so too. Nothing enforced it: not `.husky/pre-push`, not `.lintstagedrc.json`, not the Makefile, not a single CI job. When #241 finally diffed the pairs, `duckdb-service/services/discord.py` had **already** drifted from its twin — it was missing the PEP 8 blank line separating the stdlib and third-party import groups. Harmless in itself, which is precisely why it survived: it was never going to announce itself.

**Why it happened.** The duplication was a deliberate, well-reasoned trade-off, and the reasoning got written down carefully — which made it _feel_ handled. But the artefact that recorded the decision (a docstring) has no ability to enforce the invariant the decision depends on. Worse, each service's pytest suite only imports its own copy, so both suites stay green no matter how far the copies diverge; the test layer that looks like it covers these modules structurally cannot see the property that matters. `log_ring.py` is the secret-redaction layer, so the realistic failure is a redaction fix landing in one service, passing CI, and leaving the other service quietly leaking. Git history shows every change so far landed in both copies in the same commit — so this was discipline holding, not tooling, and discipline that has held is indistinguishable from tooling that exists right up until it isn't.

**How to avoid it next time.** When you write "kept in sync", "must match", or "single source of truth" in a comment or a doc, treat that sentence as a **specification for a check that does not exist yet** — either write the check in the same PR or say plainly that it is unenforced. A prose invariant with no gate is a wish. The repo already had the idiom for this (six `check-*.sh` guards wired into pre-push, a Makefile target, and a CI job each); the gap was never mechanism, only noticing that the claim needed one. Note also the second-order lesson: the gate the fix added, [`scripts/check-python-twins.sh`](../../scripts/check-python-twins.sh), deliberately implements its docstring-stripping in `awk` rather than shelling out to Python — a guard that silently no-ops because it could not find an interpreter (see the `python3` Windows-stub bug, #270) is worse than no guard, because it reports OK.

### Hooks are not gates — and the one push that reaches the fleet ran no hooks at all (#210)

**What happened.** `.husky/pre-push` ran six guard scripts; three of them (`check-no-hardcoded-api-keys.sh`, `check-stale-display-name-rule.sh`, `check-stale-reset-prose.sh`) had no CI twin, so they existed only on machines where husky was installed and only for contributors who did not pass `--no-verify`. CLAUDE.md forbids `--no-verify`, which is a request, not a control. The sharper finding is structural rather than behavioural: `scripts/deploy.sh` installs with `HUSKY=0` — correctly, since hooks on the deploy host would point `core.hooksPath` at `.husky` for a root-run unattended process — and then, in `publish_firmware`, commits the `VERSION`/`SEQUENCE` auto-bump and pushes it to `production` from that same host. So the single commit that actually ships firmware to modules in the field was the least-checked commit in the repo: no pre-push hooks by construction, and no CI either, because `production` was not in the workflow's push triggers.

**Why it happened.** Each guard was added at the moment an incident made it obvious, and pre-push is the fastest place to put one — the feedback is immediate and it costs no Actions minutes. Three of the six were then simply never promoted to CI, and nothing tracked the difference between "we have a guard for this" and "the guard runs where it matters". The secret scan in particular reads as a security control in the docs while being, in effect, advisory. The deploy-host hole is a good example of a gap that is invisible from inside either half of the system: the deploy script's `HUSKY=0` is well-commented and correct on its own terms, and the workflow's trigger list is unremarkable on its own terms — only reading both together shows that a whole class of commit escapes every check.

**How to avoid it next time.** Every guard that protects a security or correctness property needs a CI twin, and adding a pre-push hook should be treated as half of the work, not all of it ([`ci-gates.md`](../10-quality-requirements/ci-gates.md) now carries an explicit "What CI does _not_ check" section for exactly this). When auditing coverage, enumerate the **actors** who can push, not just the branches — a human developer, a fork PR, a web IDE, and an unattended root process on the production host have four different sets of checks applied to them, and only the first is the one anybody pictures.

**Fix.** The `repo-guards` CI job (#210) now runs all four of the previously-unbacked scripts (`check-no-hardcoded-api-keys.sh`, `check-stale-display-name-rule.sh`, `check-stale-reset-prose.sh`, `check-python-twins.sh`) on every push to `main`, `production`, and PRs to `main`. The `production` trigger means the firmware auto-bump commit from `scripts/deploy.sh` is finally seen by the same guards that check every human-authored commit — but a red run there is still an alert, not a gate, because the push has already landed. Enforced promotion is [#237](https://github.com/schutera/highfive/issues/237).

### `tsc` ran for the first time on the production host, with its output discarded (#208)

**What happened.** CI ran vitest for `backend/` and `homepage/`, which reads as type coverage and is not: vitest compiles through esbuild, which strips TypeScript types without checking them, and the dev server runs under `tsx` the same way. The prod `backend/Dockerfile` does run `npm run build` (i.e. `tsc`), but CI never built it — both compose stacks CI uses point at `backend/Dockerfile.dev`. `backend/tsconfig.json` also had `include: ["src/**/*"]`, so `backend/tests/*.test.ts` was outside the type-checker's scope entirely. The net effect: the first `tsc` a backend change met was `scripts/deploy.sh`'s `npm --prefix backend run build`, with output redirected to `/dev/null` — on the live host, after the commit had already been promoted to `production`. A type-broken change merged green, shipped, and then failed the deploy timer every two minutes with a reason-less rollback message. Separately, `homepage/src` contained several `eslint-disable` comments suppressing rules that no linter was running, because the repo had no ESLint config or dependency at all.

**Why it happened.** "There is a test job for that workspace" and "types are checked" are different claims that look identical on a green PR page. ADR-004's guarantee that contract drift "becomes a TypeScript compile error" was true about the compiler and false about the pipeline — nothing in CI ever invoked it. The `eslint-disable` comments are the same illusion in miniature: they are evidence that someone once thought about a rule, and they read like the rule is enforced.

**How to avoid it next time.** Ask where in the pipeline a tool _actually executes_, not whether the repo is configured for it — and distrust any quality claim whose enforcement point is downstream of merge. If the earliest place a check runs is the deploy script, it is not a gate, it is a post-mortem. Two concrete tells worth grepping for: a suppression comment (`eslint-disable`, `# type: ignore`, `# noqa`) for a tool with no config file anywhere is proof the tool is not running; and a `tsconfig.json` whose `include` omits the test directory means the test files are unchecked by _both_ the compiler and the test runner.

### Three scripts, three interpreter probes, and the one that was wrong was masked by an unrelated line (#273)

**What happened.** Three shell scripts each hand-rolled a Python-interpreter probe with three different candidate lists and two different validation techniques, and a fourth opted out of Python entirely because of that fragility. `ESP32-CAM/build.sh` tried only `python3` and `python` — never `py`. On a stock Windows 11 + Git Bash box neither of the first two resolves and only the launcher does, so the fallback path would abort on a machine with a working Python 3.12. It never fired, because `build.sh` prefers `esptool.exe` over `esptool.py` and skips the interpreter whenever it finds one, and arduino-cli's esp32 core does ship the `.exe` on Windows. The reason the bug could not happen was a coincidence in a different part of the script, not anything the probe guaranteed.

**Why it happened.** Each probe was written at the moment one script needed it, and each is individually defensible — the divergence is only visible when the three are read side by side, which nothing prompted anyone to do. Worse, the comment block in `check-duckdb-bind-claims.sh` asserted parity with `build.sh` (*"the same detection ESP32-CAM/build.sh does"*) and that claim was false when it was written: `build.sh` had validated by execution since #99, while that loop took `command -v` at its word until #270. So the one artefact that might have prompted the comparison instead recorded that the comparison had already been made. #272 then added a paragraph directly beneath it explaining that a stock Windows install *does* put a `python3` stub on PATH — leaving two adjacent statements that could not both be true.

**How to avoid it next time.** A near-duplicate of a platform workaround is worth counting, not just reviewing: the third copy is the signal, because by then the divergence is guaranteed and nobody is reading all three. The sharper lesson is about masked bugs — when a defect is unreachable, check *what* makes it unreachable. Here the mask (`esptool.exe` exists on Windows) lives in a different concern from the defect (the probe's candidate list) and could be changed by someone with no reason to look at the probe at all. "Currently unreachable" is a property of today's code, not a fix. And the parity claim is the same failure CLAUDE.md's *never trust commit messages over code* rule exists to catch, one level up: a comment asserting that two files agree is a claim that needs a mechanism, exactly like the *"kept in sync"* docstrings in #241.

**Fix.** One helper, [`scripts/lib/python.sh`](../../scripts/lib/python.sh), with a single candidate list and a single functional probe, sourced by all three scripts. It exposes an array rather than a string so `py -3` splits without `# shellcheck disable=SC2086` at every call site. The shellcheck CI job now runs with `-x` and includes `scripts/lib/*.sh`, so the shared helper is linted and each `source` is analysed in its caller's context — an unlinted helper sourced by three linted scripts would have re-created the gap in a new shape.

### GitHub's auto-close scanner matched a `closes #N` pattern inside a commit body that was quoting it as a warning (PR #104)

**What happened.** PR #104's fix commit body explained the auto-close-keyword hazard by quoting the buggy pattern verbatim: `(closes #100, #99)`. GitHub's merge-time issue-closing scanner regex-matches `close[sd]?` / `fix(es|ed)?` / `resolve[sd]?` next to `#N` anywhere in a merging PR's title, body, and every commit message it carries — it does not parse surrounding prose, quotation, or code-fence context. The quoted example closed issue #100 **and** #97 on merge, neither of which the PR actually resolved.

**Why it happened.** The commit body was written to *document* the bug pattern, not to trigger it — the intent was "here is an example of what NOT to write" — but GitHub's scanner has no concept of intent or negation; it matches the literal text sequence wherever it appears. A pattern demonstrated inside prose explaining why it's dangerous is exactly as live as the same pattern written unironically.

**How to avoid it next time.** Never write `close[sd]?` / `fix(es|ed)?` / `resolve[sd]?` next to `#N` in a commit *body* or PR *description* — including inside a quoted example — only in a PR title or the subject line of the implementing commit. Use `addresses` / `references` / `for #N` (no keyword) when discussing the mechanism itself. Verify before push: `git log <merge-base>..HEAD --pretty=full | grep -nE "(close[sd]?|fix(es|ed)?|resolve[sd]?)\s+#"` — every match must land in a subject line or PR title, never a body paragraph. This incident is the origin of the standing rule in CLAUDE.md's "Critical rules".

### A repo-policy parity checker that walked a directory on disk false-flagged a host-installed skill as repo drift

**What happened.** While building `scripts/check-agent-context.sh` (the Claude Code ↔ Codex CLI context-parity gate, [ADR-033](../09-architecture-decisions/adr-033-claude-codex-agent-context-parity.md)), the first version of its skill-mirror check enumerated every directory under `.claude/skills/` on disk and required a matching directory under `.agents/skills/`. It immediately failed on `.claude/skills/skill-creator` — a Claude Code plugin skill materialized locally on the dev machine, gitignored by `.claude/skills/*`'s existing allow-list pattern and not part of this repo at all. The check reported it as "missing from .agents/skills" — real-looking drift that wasn't.
