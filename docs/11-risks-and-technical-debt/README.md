...
2. **Removing the prose entry is part of the work.** If the rule is
   ...
   entry stays because its enforcement is partial (only
   `NODE_ENV=production` activates the guard, but production-adjacent
   environments like `staging` rely on operator discipline).
