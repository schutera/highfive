...
**Fix (`carpenter` / OTA seq 4).** `attemptGeolocation` now bounds every blocking step under the WDT budget — `WiFiClientSecure::setHandshakeTimeout(8)` (vs the 120 s default), `HTTPClient::setConnectTimeout(8000)` and `setTimeout(8000)` — plus defense-in-depth: `WiFi.scanDelete()` before the handshake and a free-heap preflight that **defers** (keeps the module online) rather than attempting a TLS handshake below a heap floor. A `[geo] free heap=…` serial log was added to tune the floor against real numbers. **Confirm on one bench module via serial (reset reason + the issue-#42 breadcrumb) before the fleet OTA** — the mechanism is strongly evidenced from telemetry but was not reproduced on a device. Do **not** roll back to `mining`: `allow_downgrade:false` and seq 4 > 3 mean the fleet moves forward only. Related: "TASK_WDT in `postImage:read_body`" above (same WDT-on-unbounded-network-read family) and "Pinned `GTS Root R1` for geolocation".
...
**Lesson.** Placeholder telemetry that looks like a plausible real value (a 0–100 "percentage") is worse than no telemetry — it actively misleads diagnosis and the dashboard. A sentinel that's obviously "no data" (a constant `0`, a `null`, an explicit "n/a") cannot be mistaken for a reading.

**Fix (`carpenter`).** Both sites now emit a `0` sentinel with a `SCAFFOLDING` comment; the wire field and duckdb column are kept so real sensing can drop in later. The homepage `BatteryHistoryChart` was already disabled (`ModulePanel.tsx`); its comment was updated to match. Re-enable end-to-end only once real sensing exists.
