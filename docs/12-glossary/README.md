...
| Term                | Definition                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Aliases to avoid                               |
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| **WPA2-Personal**   | The default WiFi auth scheme: a single pre-shared key (PSK / password), no username. The path every existing module uses (`WiFi.begin(ssid, password)`). Selected when the optional WiFi **username** is empty.                                                                                                                                                                                                                                                                                | PSK (in prose), home WiFi                      |
| **WPA2-Enterprise** | Optional 802.1X WiFi auth needing **both a username and a password** (eduroam, university/corporate networks). Engaged when the optional WiFi **username** is non-empty; the firmware joins via PEAP/TTLS + MSCHAPv2 (`esp_wpa2.h` enterprise APIs, then `WiFi.begin(ssid)` with no PSK). The PSK-vs-enterprise branch is `ESP32-CAM/lib/wifi_auth/`'s `hf::wifiAuthMode`. **No RADIUS server-cert validation** — see [ADR-018](../09-architecture-decisions/adr-018-wpa2-enterprise-wifi.md). | enterprise WiFi (bare), 802.1X (in prose), EAP |
| **WiFi Username**   | The optional onboarding field selecting WPA2-Personal vs -Enterprise. Captive-portal "WiFi Username (optional, for enterprise networks)" input; `NETWORK.USERNAME` in `config.json`. Empty (incl. whitespace-only) → PSK. Used as both the EAP identity and the inner MSCHAPv2 username (no separate outer identity).                                                                                                                                                                          | identity, eap_identity, login (in field names) |
...
| **Measurement**           | One row in the `measurements` table (issue #110, see [ADR-016](../09-architecture-decisions/adr-016-per-module-measurements-store.md)). Five-tuple `(module_mac, ts, metric, value, source)`. The canonical per-module time-series store; downstream baseline (#115), anomaly (#116), and hatching-prediction (#117) features read from this. Append-only, no PK, no FK. **Caveat for `battery_pct`:** the firmware currently writes `random(1, 100)` for battery (issue #8a) — measurements records what the firmware sends honestly; the noise resolves when #8a / #8b land.                                                       | metric row, time-series row, measurement record |
