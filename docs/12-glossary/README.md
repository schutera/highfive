...
| **Bee-name version**     | Firmware version identifier of the form `bumblebee` / `honeybee` / `mason` / `carpenter`. **Currently three uncoordinated sources** — `ESP32-CAM/VERSION` (manifest), `FIRMWARE_VERSION` macro in `esp_init.h:8` (sidecar), `FW_VERSION` macro in `client.cpp:232` (heartbeat body). See [ADR-006](../09-architecture-decisions/adr-006-bee-name-firmware-versioning.md) and chapter 11 tech-debt. | firmware tag                        |
...
| **Circuit Breaker (ESP)**| `static uint8_t consecutiveFailures` in `captureAndUpload` (`ESP32-CAM.ino:222`). Counts consecutive **upload-path** failures of any kind (camera NULL, WiFi down, HTTP non-2xx); at >= 5 it calls `delay(1000); ESP.restart()` immediately. The heartbeat status code is **not** wired in — only upload outcomes are counted. See [ADR-007](../09-architecture-decisions/adr-007-esp-reliability-breaker-and-daily-reboot.md). | failure counter                     |
| **Daily Reboot**         | The 24-hour-uptime auto-restart, with NVS-flagged capture-skip on the wake boot so the daily image cost isn't doubled. Flag lives in NVS namespace `"boot"` key `daily_reboot` (`ESP32-CAM.ino:186, 306`). | scheduled reboot                    |
