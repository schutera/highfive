...
| `last_hb_fail_code`  | int    | optional (#172) — the return value of the most recent _failed_ heartbeat: `-2` = connect/WiFi-down, `-4` = unparseable status line, otherwise the raw non-2xx HTTP code. Only attached when a prior streak exists.                                                                                                                                                                                                                                                          |
| `last_hb_fail_count` | int    | optional (#172) — consecutive heartbeat failures since the last 2xx. The hourly heartbeats fail _between_ boots and never reach the server (no 2xx), so this streak is carried forward and reported on the next 2xx heartbeat (typically the boot heartbeat after a `livenessReboot`). A non-zero value on an otherwise-online module is the #170 reboot-loop signature made remotely visible. Persisted across software resets in RTC memory (`ESP32-CAM/lib/hb_failure`). |
