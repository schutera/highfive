...
def test_get_modules_returns_json_500_on_query_failure(client, monkeypatch):
    """Pin behaviour from issue #32, generalised by #246: an uncaught DB
    exception must surface as parseable JSON with status 500, not the
    Flask default HTML page that the Node backend can't deserialize
    (and would have masked as a generic 502). The body is the generic
    envelope — the detail goes to the ring, never `str(e)` to the
    client."""
