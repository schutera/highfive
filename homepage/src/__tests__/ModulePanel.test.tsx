...
vi.mock('../services/api', () => ({
  api: {
    ...
    // Per-nest snip grid + global time-lapse (#165 / #166). Defaults to no
    // history so this suite focuses on the panel chrome; the NestSnipGrid
    // component has its own test file.
    getSnipHistory: vi.fn().mockResolvedValue([]),
    getSnipUrl: vi.fn(
...
// The full-module "Latest captures" gallery was removed from ModulePanel in
// favour of the per-nest time-lapse grid (#166). The snip grid has its own
// test file (NestSnipGrid.test.tsx); this suite stubs getSnipHistory to an
// empty list so the panel-level tests above stay isolated from it.
