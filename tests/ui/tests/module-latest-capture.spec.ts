...
// "Latest capture" card in the public ModulePanel (#154 phase 1) —
// CLAUDE.md rule #4: a new view rendering wire-shape data gets a
// Playwright spec against the production-built homepage + real backend.
// The jsdom suite (ModulePanel.test.tsx "latest capture") pins the
// component against a mocked ImageUploadsPage; only this layer proves
// the real chain — duckdb-service uploaded_at → image-service bytes →
// backend proxy → nginx → <img> pixels — actually renders, which jsdom
// can never do (it does not load images at all).
//
// Reuses the admin-gallery seed: seed_ui_fixtures.py uploads 6 images
// for GALLERY_MAC ("UI Test Gallery"), so the newest one is this card's
// expected content. The expected filename is derived from the live API,
// not from seed timing, so same-second upload collisions can't flake it.

const GALLERY_MAC = 'ff2222222222';

test.describe('module panel latest capture', () => {
  test('renders the newest upload with timestamp, opens a delete-free lightbox', async ({
    page,
  }) => {
    ...
    await expect(panel.getByText(/Latest capture|Neueste Aufnahme/)).toBeVisible();
