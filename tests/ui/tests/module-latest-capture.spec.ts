...
// NOTE: this spec proves the flag-ON render path only. The UI-test stack
// builds the homepage with VITE_ENABLE_DASHBOARD_IMAGES=true (ADR-022); the
// flag-OFF "panel hides the gallery" path has no automated proof here — it
// rests on flagEnabled()'s `=== 'true'` semantics (unit-tested in
// homepage/src/__tests__/featureFlags.test.ts) and the prod Dockerfile's
// empty default.
//
